...
router.post('/saveorder', function(req, res, next)
{
 ...
 router.get('/updateOrderNumber', function(req,res,next)
 {
    console.log('update order number');
      db.vendorOrder.update({_id:mongojs.ObjectId('58fbd6f52139ee0aa962991b')}, {$inc:{vendorOrderNumber:1}});
...
{
    db.vendorOrder.findOne({_id:mongojs.ObjectId('58fbd6f52139ee0aa962991b')},function(err,vendorOrderNumber){
        if(err){
            res.send(err)
        }
