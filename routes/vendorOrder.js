...
router.post('/saveorder', function(req, res, next)
{
 ...
 router.get('/updateOrderNumber', function(req,res,next)
 {
    console.log('update order number');
      db.vendorOrder.update({_id:mongojs.ObjectId('58fbfd4f8e601a185c2edf2f')}, {$inc:{vendorOrderNumber:1}});
...
{
    db.vendorOrder.findOne({_id:mongojs.ObjectId('58fbfd4f8e601a185c2edf2f')},function(err,vendorOrderNumber){
        if(err){
            res.send(err)
        }
